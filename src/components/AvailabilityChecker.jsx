...
const TimeSlots = ({ date, bookingsForDate, onSelect, onBack }) => {
    const slots = ["09:00", "10:00", "11:00", "12:00", "13:00", "14:00", "15:00", "16:00", "17:00"];
    const [startTime, setStartTime] = useState('');
    const [endTime, setEndTime] = useState('');

    const isSlotBooked = (slot) => {
        return bookingsForDate.some(b => b.startTime <= slot && b.endTime > slot && b.status !== 'rejected');
    };
    ...
    return (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="w-full lg:max-w-xs bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl">
            <button onClick={onBack} className="text-sm mb-4 text-indigo-500 hover:underline">&larr; Back to calendar</button>
...
export default function AvailabilityChecker({ bookings, onSlotSelect }) {
    ...
    return (
        <motion.div key="calendar-view" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex flex-col lg:flex-row gap-8">
            <div className="flex-grow bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-lg shadow-xl">
                ...
                <div className="grid grid-cols-7 gap-1 sm:gap-2 text-center">
                    {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, index) => <div key={`${day}-${index}`} className="font-semibold text-xs sm:text-sm">{day}</div>)}
                    ...
                    {days.map(day => {
                        ...
                        return (<div key={day} onClick={() => setSelectedDate(dayDate)} className={`p-2 rounded-full cursor-pointer transition-colors text-sm ${isSelected ? 'bg-indigo-600 text-white' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}`}>{day + 1}</div>);
                    })}
                </div>
            </div>
            {selectedDate && <TimeSlots date={selectedDate} bookingsForDate={bookings[formatDate(selectedDate)] || []} onSelect={onSlotSelect} onBack={() => setSelectedDate(null)} />}
        </motion.div>
    );
};